...
def calculate_velocity(actor, velocities):
    """
    Method to calculate the velocity of a actor
    """
    vel_xyx = velocities[actor.id]
    
    velocity_squared = vel_xyx.x**2
    velocity_squared += vel_xyx.y**2
...
class ActorBlockedTest(Criterion):
    ...
    def __init__(self, actor, min_speed, max_time, name="ActorBlockedTest", optional=False, terminate_on_failure=True):
        """
        Class constructor
        """
...
class RunningStopTest(Criterion):
    ...
    def _scan_for_stop_sign(self, actor_transform, wp_list, velocities):
        """
        ...
        actor_velocity = velocities[self.actor.id]
    ...
    def update(self, velocities):
        ...
        if not self._target_stop_sign:
            self._target_stop_sign = self._scan_for_stop_sign(actor_transform, check_wps, velocities)
            return new_status

        if not self._stop_completed:
            current_speed = calculate_velocity(self.actor, velocities)
