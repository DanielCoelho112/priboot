...
def prettify_json(args):
    ...
    if records_table:
        ...
        for route in records_table:
            ...
            metrics_route = [[key, '{:.6f}'.format(values), ''] for key, values in route['scores'].items()]
            infractions_route = [[key, '{:.6f} ({} occurrences)'.format(len(values)/route_completed_kms, len(values)),
                                 '\n'.join(values)] for key, values in route['infractions'].items()]

            times = [['Game duration', '{:.6f}'.format(route['meta']['duration_game']), 'seconds'],
                     ['System duration', '{:.6f}'.format(route['meta']['duration_system']), 'seconds']]

            route_completed_length = [ ['distance driven', '{:.6f}'.format(route_completed_kms), 'Km']]
        ...
        list_statistics.extend([['Total game duration', '{:.6f}'.format(total_duration_game), 'seconds']])
        list_statistics.extend([['Total system duration', '{:.6f}'.format(total_duration_system), 'seconds']])
        list_statistics.extend([['Total distance driven', '{:.6f}'.format(total_route_length), 'Km']])
