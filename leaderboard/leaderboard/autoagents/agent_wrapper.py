...
from utilities.controls import carla_control
from utilities.controls import acc_controller
...
from leaderboard.envs.birdview import BirdviewReader
...
class AgentWrapper(object):
    ...
    def random_action(self):
        ...
        # we need this to get this for getting the data to replay buffer later.
        self._agent.update_input_data()
        
        action = np.asarray([random.uniform(0, 1), random.uniform(-1, 1)])        
        controls = carla_control(acc_controller(action=action))
        
        return action, controls, {}
    
    def get_obs(self):
        return self._agent.get_obs()
    
    def push_transition(self, obs, action, reward, done):
        self._agent.push_transition(obs, action, reward, done)
    ...
    def setup_sensors(self, vehicle, criteria_node):
        """
        ...
        for sensor_spec in self._agent.sensors():
            ...
            if type_ == 'sensor.opendrive_map':
                ...
                sensor = BirdviewReader(vehicle, attributes['reading_frequency'], configs=self._agent.configs, criteria_node=criteria_node, agent=self._agent, client=client, cache_dict=self._agent.cache_dict)
