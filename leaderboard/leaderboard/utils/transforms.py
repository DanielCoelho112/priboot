...
def loc_global_to_ref_numpy(target_loc_in_global, ref_trans_in_global):
    """
    Convert a location from global coordinates to reference frame coordinates using numpy arrays.
    
    :param target_loc_in_global: numpy array representing location in global coordinates (x, y, z).
    :param ref_trans_in_global: dict with 'location' as a numpy array (x, y, z) and 'rotation' as a carla.Rotation object.
    :return: numpy array representing location in reference frame coordinates.
    """
    # Subtract reference location from target location to get the vector in global coordinates
    vec_in_global = target_loc_in_global - ref_trans_in_global['location']
    vec_in_ref = vec_global_to_ref_numpy(vec_in_global, ref_trans_in_global['rotation'])
    return vec_in_ref
...
def vec_global_to_ref_numpy(target_vec_in_global, ref_rot_in_global):
    """
    Convert a vector from global coordinates to reference frame coordinates using numpy arrays.
    
    :param target_vec_in_global: numpy array representing the vector in global coordinates (x, y, z).
    :param ref_rot_in_global: carla.Rotation object for the reference frame in global coordinates.
    :return: numpy array representing the vector in reference frame coordinates.
    """
    R = carla_rot_to_mat_numpy(ref_rot_in_global)
    np_vec_in_ref = R.T.dot(target_vec_in_global)
    return np_vec_in_ref
...
def carla_rot_to_mat_numpy(carla_rotation):
    """
    Convert a CARLA rotation to a rotation matrix using numpy arrays.
    
    :param carla_rotation: carla.Rotation object.
    :return: numpy array representing the rotation matrix.
    """

    roll, pitch, yaw = np.deg2rad([carla_rotation['roll'], carla_rotation['pitch'], carla_rotation['yaw']])
...
def get_loc_rot_vel_in_ev(actor_list, ev_transform):
    location, rotation, absolute_velocity, type = [], [], [], []
    for actor in actor_list:
        ...
        type.append(actor.type_id)
    return location, rotation, absolute_velocity, type

def get_loc_rot_in_ev(actor_list, ev_transform):
    location, rotation, absolute_velocity = [], [], []
    for actor in actor_list:
        # location
        location_in_world = actor.location
        location_in_ev = loc_global_to_ref(location_in_world, ev_transform)
        location.append([location_in_ev.x, location_in_ev.y, location_in_ev.z])
        # rotation
        rotation_in_world = actor.rotation
        rotation_in_ev = rot_global_to_ref(rotation_in_world, ev_transform.rotation)
        rotation.append([rotation_in_ev.roll, rotation_in_ev.pitch, rotation_in_ev.yaw])
