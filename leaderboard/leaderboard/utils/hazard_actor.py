...
from leaderboard.envs.bev_utils.actors import BIKE_TYPES
...
def lbc_hazard_vehicle(obs_surrounding_vehicles, ev_speed=None, proximity_threshold=9.5):
    for i, is_valid in enumerate(obs_surrounding_vehicles['binary_mask']):
        ...
        with_distance_ahead = is_within_distance_ahead(sv_loc, proximity_threshold, up_angle_th=15)
        if same_heading and with_distance_ahead:
            return sv_loc
    return None

def lbc_hazard_dynamic(obs_surrounding_vehicles, ev_speed=None, proximity_threshold=9.5):
    for i, location in enumerate(obs_surrounding_vehicles['location']):

        sv_yaw = obs_surrounding_vehicles['rotation'][i][2]
        same_heading = abs(sv_yaw) <= 150
        
        sv_loc = obs_surrounding_vehicles['location'][i]
        with_distance_ahead = is_within_distance_ahead(sv_loc, proximity_threshold, up_angle_th=10)
...
def lbc_hazard_walker(obs_surrounding_pedestrians, ev_speed=None, proximity_threshold=20.0):
...
def lbc_hazard_construction(obs_surrounding_construction, ev_speed=None, proximity_threshold=9.5):
    for i, is_valid in enumerate(obs_surrounding_construction['binary_mask']):
        if not is_valid:
            continue

        sv_yaw = obs_surrounding_construction['rotation'][i][2]
        same_heading = abs(sv_yaw) <= 150

        sv_loc = obs_surrounding_construction['location'][i]
        with_distance_ahead = is_within_distance_ahead(sv_loc, proximity_threshold, up_angle_th=10)
        if same_heading and with_distance_ahead:
            return sv_loc
    return None
...
def challenge_hazard_vehicle2(obs_surrounding_vehicles, ev_speed):
    # np.linalg.norm(_numpy(self._vehicle.get_velocity())
    o1 = np.float32([1, 0])
    p1 = np.float32([0, 0])
    s1 = max(9.5, 2.0 * ev_speed)
    ...
    for i, is_valid in enumerate(obs_surrounding_vehicles['binary_mask']):
        ...
        sv_yaw = obs_surrounding_vehicles['rotation'][i][2]
        sv_vel = obs_surrounding_vehicles['absolute_velocity'][i]
        sv_vel_mag = np.linalg.norm(sv_vel[0:2])
        sv_type = obs_surrounding_vehicles['type'][i]
        
        # if it is bike we need to come closer.
        is_bike = False
        ...
        if angle_between_heading > 60.0 and not (angle_to_car < 15 and distance < s1):
            ...
            continue
        elif angle_to_car > 5.0 and sv_vel_mag < 0.1:
            continue
        elif ev_speed < 0.1 and angle_to_car > 5.0:
            continue
        elif is_bike and angle_between_heading < 45:
            continue
